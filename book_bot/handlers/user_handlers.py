...
from book_bot.database.database import users_db, user_reading_status
...
async def process_start_command(message: Message):
    ...
    if message.from_user.id not in users_db:
        users_db[message.from_user.id] = deepcopy(user_reading_status)
...
async def process_beginning_command(message: Message):
    ...
    await message.answer(
        text=text,
        reply_markup=create_pagination_keyboard(users_db[message.from_user.id]['page'], len(book)))
...
async def process_forward_press(callback: CallbackQuery):
    if users_db[callback.from_user.id]['page'] < len(book):
        ...
        await callback.message.edit_text(
            text=text,
            reply_markup=create_pagination_keyboard(page_num, len(book)))
...
async def process_bookmark_press(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        text=text,
        reply_markup=create_pagination_keyboard(users_db[callback.from_user.id]['page'], len(book)))
    await callback.answer()
